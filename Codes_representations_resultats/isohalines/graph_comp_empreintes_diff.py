...
diff_file_1psu = '/lus/store/CT1/c1601279/aperez/resultats/diff/diff_12PSOURCE_1PSU_25deg.nc'
diff_file_15psu = '/lus/store/CT1/c1601279/aperez/resultats/diff/diff_12PSOURCE_15PSU_25deg.nc'

seuil_panache = -0.2 #en psu, critère de définition du panache 
seuil_aberrant = 1e5
...
dates_1psu, surfaces_1psu = calculer_empreinte(diff_file_1psu, "1 PSU")
dates_15psu, surfaces_15psu = calculer_empreinte(diff_file_15psu, "15 PSU")
...
ax.plot(dates_1psu, surfaces_1psu, linewidth=2, marker='o', markersize=4,
        color='#1f77b4', label='13 fleuves - 1 PSU')
ax.plot(dates_15psu, surfaces_15psu, linewidth=2, marker='o', markersize=4,
        color='#ff7f0e', label='13 fleuves - 15 PSU')
...
# Titres et labels
ax.set_ylabel(f"Empreinte du panache [km²]\n(ΔSSS ≤ {seuil_panache} psu)", fontsize=11)
ax.set_xlabel("Date", fontsize=11)
ax.set_title("Évolution temporelle de l'empreinte spatiale des panaches des 13 fleuves (écart à la référence)",
...
fig.tight_layout()
plt.show()
fig.savefig('empreinte_panache_diff_13fleuves_1psu_15psu.png', dpi=300)
